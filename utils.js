function marketDepth(orderBook, steps = [1, 5, 10, 20, 30], prefix = "") {
  ...
  for (let i = 0; i < orderBook.length && currentStep < steps.length; i++) {
    const currentDepth = Math.abs(1 - orderBook[i][0] / surfacePrice)

    if (currentDepth >= steps[currentStep] / 100) {
      result[`${prefix}${steps[currentStep]}_percent_depth`] = currentVolume
      currentStep += 1
    }
